...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create option" do
    assert_difference('Option.count') do
      post options_url, params: { option: { poll_id: @option.poll_id, title: @option.title } }
    end
  ...
  test "should update option" do
    patch option_url(@option), params: { option: { poll_id: @option.poll_id, title: @option.title } }
    assert_response 200
  end
